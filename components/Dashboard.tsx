...
export function Dashboard({ result, readOnly = false }: { result: AnalysisResult; readOnly?: boolean }) {
  ...
  const direct = result.strategicPlaces.filter((p) => p.categoriaEstrategica === 'Concorrente direto de tecnologia').length;
  const indirect = result.strategicPlaces.filter((p) => p.categoriaEstrategica === 'Concorrente indireto extracurricular').length;
  const position = result.posicionamentoUnidade || result.posicionamentoMyRobot;
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
      ...
      <div id="analysis-report" className="space-y-6">
        ...
        <section id="obstaculos"><Card><h2 className="text-2xl font-bold text-slate-900">Obstáculos que podem atrapalhar a matrícula</h2><div className="mt-5 grid gap-4">{result.obstaculosMatricula.length ? result.obstaculosMatricula.map((item, index) => <div key={index} className="rounded-2xl border border-slate-200 p-4"><div className="flex flex-wrap items-center gap-2"><AlertTriangle className="h-4 w-4 text-orange-500" /><strong>{item.bairro} — {item.tipoObstaculo}</strong><Badge className="bg-slate-100 text-slate-700">{item.impactoEstimado}</Badge></div><p className="mt-2 text-sm text-slate-600">{item.descricao}</p>{item.evidencias.length > 0 && <ul className="mt-2 list-disc pl-5 text-sm text-slate-600">{item.evidencias.map((ev) => <li key={ev}>{ev}</li>)}</ul>}<p className="mt-2 text-sm font-semibold text-slate-800">Ação: {item.acaoRecomendada}</p></div>) : <p className="text-sm text-slate-600">Nenhum obstáculo relevante foi identificado no raio analisado.</p>}</div></Card></section>
        ...
        <section id="personas"><Card><h2 className="text-2xl font-bold text-slate-900">Personas do Público-Alvo</h2><p className="mt-2 text-sm text-slate-500">As personas combinam o responsável e o filho, porque ambos influenciam a decisão de matrícula.</p><div className="mt-5 grid gap-4 md:grid-cols-2">{result.personas.map((persona) => <div key={persona.nomeFicticio} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-orange-100 text-lg font-bold text-orange-700">{persona.nomeFicticio[0]}</div><div><h3 className="font-bold text-slate-900">{persona.nomeFicticio}</h3><p className="text-sm text-slate-500">{persona.perfilFamiliar}</p></div></div><div className="mt-3 space-y-2 text-sm text-slate-700"><p><strong>Filho:</strong> {persona.filhoIdade} · {persona.filhoPerfil}</p><p><strong>Papel do filho na decisão:</strong> {persona.papelDoFilhoNaDecisao}</p><p><strong>Mensagem:</strong> {persona.mensagemRecomendada}</p></div></div>)}</div></Card></section>
