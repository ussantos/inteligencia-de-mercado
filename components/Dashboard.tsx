  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
      ...
      <div id="analysis-report" className="space-y-6">
        ...
        <section id="plano"><Card><h2 className="text-2xl font-bold text-slate-900">Plano de Ação — Próximos Passos</h2><p className="mt-2 text-sm text-slate-500">Quando a chave OpenAI está configurada, esta seção é enriquecida por IA com base nos bairros, concorrentes, CNAEs, raio de análise e limitações encontradas.</p><div className="mt-5 grid gap-4">{result.planoDeAcao.map((item) => <div key={item.prioridade} className="rounded-2xl border border-slate-200 p-4"><div className="flex flex-wrap gap-2"><Badge className="bg-orange-100 text-orange-700">Prioridade {item.prioridade} · {item.tipo}</Badge><Badge className="bg-slate-100 text-slate-700">Impacto {item.impactoEsperado}</Badge><Badge className="bg-slate-100 text-slate-700">Execução {item.facilidadeExecucao}</Badge></div><h3 className="mt-3 font-bold text-slate-900">{item.acao}</h3><div className="mt-3 grid gap-2 text-sm text-slate-600 md:grid-cols-2"><p><strong>Prazo:</strong> {item.prazoSugerido}</p><p><strong>Custo:</strong> {item.custoEstimado}</p><p><strong>Responsável:</strong> {item.responsavelSugerido}</p><p><strong>KPI:</strong> {item.kpiParaMedirSucesso}</p></div></div>)}</div></Card></section>
